...
import { Search, Filter, Plus, X, Upload, Download, Trash2, Edit, ChevronRight, FileText, Calendar, DollarSign, User, Building2, MapPin, Send, AlertCircle } from 'lucide-react';
...
export default function OpportunitiesPage() {
        ...
        const { data, error } = await supabase
            ...
            .select(`
                *,
                ...
                team_members:sales_rep_id (first_name, last_name)
            `)
    ...
    const filterOpportunities = () => {
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter(opp =>
                ...
                `${opp.contact_first_name} ${opp.contact_last_name}`.toLowerCase().includes(term) ||
                opp.companies?.company_name?.toLowerCase().includes(term) ||
                opp.site_address?.toLowerCase().includes(term) ||
                opp.sub_status?.toLowerCase().includes(term)
            );
        }
    ...
    const deleteAttachment = async (attachmentId: string, filePath: string) => {
        ...
        try {
            ...
            // Remove from storage (optional - can keep for recovery)
    ...
    const sendToVA = async () => {
        ...
        try {
            ...
            await supabase
                .from('opportunities')
                .update({ 
                    recommendation_status: 'Not Started',
                    updated_at: new Date().toISOString()
                })
                .eq('id', opportunityToSend.id);

            // TODO: Send email to VA (requires email service setup)
            // const vaEmail = vaList.find(va => va.id === selectedVA)?.email;
            // await sendEmailToVA(vaEmail, opportunityToSend);
    ...
    const getCustomerName = (opp: Opportunity) => {
        if (opp.companies?.company_name) {
            return opp.companies.company_name;
        }
        return `${opp.contact_first_name} ${opp.contact_last_name}`;
    };

    const getContactPerson = (opp: Opportunity) => {
        if (opp.companies?.company_name) {
            return `${opp.contact_first_name} ${opp.contact_last_name}`;
        }
        return null;
            ...
            <div className="bg-white border-b border-gray-200 px-6 py-4">
                <div className="flex items-center justify-between">
                    ...
                    <Link
                        ...
                        className="px-4 py-2 bg-[#0066CC] hover:bg-[#0052a3] text-white rounded-lg flex items-center gap-2"
                    >
                                            ...
                                            {opp.stage === 'QUALIFIED' && (
                                                <button
                                                    onClick={(e) => {
                                                        ...
                                                        // Auto-select VA if only one exists
                                                        if (vaList.length === 1) {
                                ...
                                <div className="space-y-6">
                                        ...
                                        <div className="space-y-2 text-sm">
                                            <div className="flex justify-between">
                                                ...
                                                <span className="font-medium">
                                                    {selectedOpportunity.contact_first_name} {selectedOpportunity.contact_last_name}
                                                </span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-600">Email:</span>
                                                <span className="font-medium">{selectedOpportunity.contact_email}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-600">Phone:</span>
                                                <span className="font-medium">{selectedOpportunity.contact_phone}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-600">Contact Type:</span>
                                                <span className="font-medium">{selectedOpportunity.contact_type}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-600">Client Type:</span>
                                                <span className="font-medium">{selectedOpportunity.client_type}</span>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-2 text-sm">
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="font-medium">{selectedOpportunity.companies.company_name}</span>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-700">
                                            {selectedOpportunity.site_address}<br />
                                            {selectedOpportunity.site_city} {selectedOpportunity.site_postcode}
                                        </p>
                                    </div>
                                    ...
                                    {/* Sales Rep */}
                                    {selectedOpportunity.team_members && (
                                        <div>
                                            <h3 className="text-sm font-semibold text-gray-800 mb-3">Sales Rep</h3>
                                            <p className="text-sm text-gray-700">
                                                {selectedOpportunity.team_members.first_name} {selectedOpportunity.team_members.last_name}
                                            </p>
                                        </div>
                                    )}
                        ...
                        <div className="sticky bottom-0 bg-white border-t border-gray-200 px-6 py-4">
                            <div className="flex gap-3">
                                ...
                                <Link
                                    ...
                                    className="flex-1 px-4 py-2 bg-[#0066CC] hover:bg-[#0052a3] text-white text-center rounded-lg flex items-center justify-center gap-2"
                                >
