...
type SortField = 'quote_number' | 'site_address' | 'city' | 'job_type' | 'status' | 'quote_date' | 'total_amount';
...
export default function QuotesPage() {
  ...
  const fetchQuotes = async () => {
    try {
      ...
      // Simple query - just get quotes without complex joins
      let query = supabase
        .from('quotes')
        .select('*', { count: 'exact' });
      ...
      const transformedData = (data || []).map((quote: any) => ({
        ...
        client_name: quote.customer_first_name && quote.customer_last_name
        ...
        created_date: quote.quote_date || quote.created_at,
        follow_up_date: '',
        sales_rep: '',
        discount_applied: false,
        total_ex_gst: quote.subtotal || 0,
        total_inc_gst: quote.total_amount || 0,
        margin: 0,
      }));
              ...
              <table className="w-full min-w-[1600px]">
                ...
                <thead>
                  <tr className="bg-[#0066CC] text-white">
                    ...
                    >
                      Quote Date <SortArrow field="quote_date" />
                    </th>
                    <th 
